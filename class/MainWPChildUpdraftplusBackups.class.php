...
//        }
    ...
    public function historystatus($remotescan = null, $rescan = null ) {
        ...
        return array('n' => sprintf(__('Existing Backups', 'updraftplus').' (%d)', count($backup_history)), 't' => $output, 'c' => count($backup_history), 'm' => $updraftplus->detect_safe_mode());
    }
...
//        error_log(print_r($res, true));
        ...
        if (isset($res['updraft_restore'])) {
                ...
                return array('m' => '<p>'.$mess_first.'</p>'.implode('<br>', $mess), 'w' => implode('<br>', $warn), 'e' => implode('<br>', $err));
        }
    }
...
// 		if (0 == $found_jobs) $ret .= '<p><em>'.__('(None)', 'updraftplus').'</em></p>';
	...
	private function print_active_job($job_id, $is_oneshot = false, $time = false, $next_resumption = false) {
		...
		if (!$is_oneshot) $ret .=' - <a title="'.esc_attr(__('Note: the progress bar below is based on stages, NOT time. Do not stop the backup simply because it seems to have remained in the same place for a while - that is normal.', 'updraftplus')).'" href="javascript:mainwp_updraft_activejobs_delete(\''.$job_id.'\')">'.__('delete schedule', 'updraftplus').'</a>';
        ...
        private function activejobs_delete() {
                    ...
                    if (isset($job['updraft_backup_resume'])) {
                            foreach ($job['updraft_backup_resume'] as $hook => $info) {
                                    if (isset($info['args'][1]) && $info['args'][1] == $jobid) {
                                            ...
                                            if (!$found_it) return array('ok' => 'Y', 'm' => __('Job deleted', 'updraftplus'));
                                            $found_it = 1;
                                    }
                            }
                    }
            ...
            if (!$found_it) return array('ok' => 'N', 'm' => __('Could not find that job - perhaps it has already finished?', 'updraftplus'));
            return array();
       }
        ...
        public function fetch_log($backup_nonce, $log_pointer=0) {
            ...
            if (!preg_match('/^[0-9a-f]+$/', $nonce)) return array('error' => 'Security check');
